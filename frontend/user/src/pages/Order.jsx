...
export default function Order() {
  ...
  const [priceList, setPriceList] = useState(null) // State untuk nyimpen Harga dari DB

  const [orderDetails, setOrderDetails] = useState({
    paperSize: 'A4',
    colorMode: 'Hitam Putih',
    copies: 1, 
    totalPages: 1, // Jumlah halaman (Otomatis kalau PDF, manual kalau Word/Gambar)
  ...
  const [isPdf, setIsPdf] = useState(false) // Buat nandain file-nya PDF bukan
  ...
  useEffect(() => {
    ...
    fetchUserData()
    fetchPrices() // Tarik harga pas halaman diload
  ...
  const handleFile = async (e) => {
    ...
    if (selectedFile) {
      ...
      // Kalau file PDF, langsung scan jumlah halamannya
      if (selectedFile.type === 'application/pdf') {
         setIsPdf(true);
         try {
             const arrayBuffer = await selectedFile.arrayBuffer();
             const pdf = await pdfjsLib.getDocument({ data: arrayBuffer }).promise;
             setOrderDetails(prev => ({ ...prev, totalPages: pdf.numPages })); // Set total halaman!
         } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        nilai_pesanan: totalHarga // KINI NGIRIM HARGA BENERAN!
      }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {selectedService.nama.toLowerCase().includes('cetak') && (
                <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {/* INPUT HALAMAN (Cuma muncul di mode normal, dan bisa diedit kalau bukan PDF) */}
        ...
        <button type="submit" disabled={loading || uploading || success} className="w-full py-4 bg-gradient-to-r from-olaTosca to-olaBlue text-white rounded-xl font-bold text-lg shadow-lg hover:shadow-xl hover:scale-[1.01] transition-all disabled:opacity-50 disabled:scale-100 flex items-center justify-center gap-2">
